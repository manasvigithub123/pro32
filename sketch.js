...
function draw() {
  ...
  if(gameState === "start") {
  		textSize(24)
  		text("Instructions: Try to knock all the bottles down with the least amount of misses and turns! Press space to get another turn",150, 125, 350)
	}
  pop()
...
async function Time() {
	...
	//puts it into the JSON formating
    var responseJSON = await response.json();
	//looks into the day of time part of the file
    var time = responseJSON.datetime;
	//slices the hours
    var data = time.slice(11,13);

	//displays different colored background depending on the time of day
    if(data > 06 && data < 20) {
